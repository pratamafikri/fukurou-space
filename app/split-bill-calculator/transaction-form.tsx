...
export default function TransactionForm() {
  ...
  const calculateAfterSplit = (rowTotal: number) => {
    const totalBeforeSplit = rows.reduce((acc, row) => acc + row.quantity * row.price, 0)
    const totalAfterSplit =
      totalBeforeSplit + totals.totalServiceCharge + totals.totalDeliveryCost - totals.totalDiscount
    ...
    const afterSplit =
      rowTotal +
      (totals.totalServiceCharge / transactionCount) +
      (totals.totalDeliveryCost / transactionCount) -
      (totals.totalDiscount / transactionCount)
